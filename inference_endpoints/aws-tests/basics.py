...
class LambdaWrapper:
    ...
    def create_iam_role_for_codebuild(self, iam_role_name):
        ...
        # Add additional permissions as needed for CodeBuild to access ECR
        # For example, permissions to read and write to your ECR repository
        codebuild_permissions_policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
                    "Effect": "Allow",
        ...
        try:
            role = self.iam_resource.create_role(
                ...
                AssumeRolePolicyDocument=json.dumps(codebuild_assume_role_policy),
            )
            logger.info("Created role %s.", role.name)

            # Attach CodeBuild permissions policy
            role.attach_policy(
                PolicyArn="arn:aws:iam::aws:policy/AWSCodeBuildDeveloperAccess"
            )
            logger.info("Attached CodeBuild developer access policy to role %s.", role.name)

            # Attach custom permissions policy for ECR access
            role.attach_policy(
                PolicyName="CodeBuildECRPolicy",
                PolicyDocument=json.dumps(codebuild_permissions_policy),
            )
            logger.info("Attached custom ECR permissions policy to role %s.", role.name)
            ...
            if error.response["Error"]["Code"] == "EntityAlreadyExists":
                ...
                logger.exception(
                    "Couldn't create role %s or attach policies.",
                    iam_role_name,
                )
                raise
    ...
    def create_function(
            self, function_name, handler_name, iam_role, deployment_package, runtime="python3.11"
    ):
        """
    ...
    def list_functions(self):
        ...
        try:
            ...
            return func_paginator.paginate().build_full_result()
        except ClientError as err:
            logger.error(
    ...
    def create_codebuild_project(self, project_name, buildspec_file='buildspec.yml', codebuild_image='aws/codebuild/standard:7.0'):
        try:
            response = self.lambda_client.create_project(
                name=project_name,
                source={
                    'type': 'NO_SOURCE',
            ...
            logger.error('Couldn\'t create codebuild project. Here\'s why: %s: %s',
                         err.response['Error']['Code'],
                         err.response['Error']['Message'])
            raise
        else:
